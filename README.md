...
静止WebPでもJPEGと同じ9項目のXMPメタデータを表示・編集し、WebPへの保存で保持・設定・削除できるようにしました。保存時のメタデータ書換えでは、圧縮画像データを再処理しません。元のEXIF・ICC・未知のXMPの完全保持や、他形式へのメタデータ移送は対象外です。現在の画像書出しは1枚出力のため、この保存経路ではアニメーションWebPを明示的に拒否します。
