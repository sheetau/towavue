...
画像切替で進行中の先読みを引き継ぎ、現在画像の表示準備と次画像の復号を並行させます。4096×2304の生成JPEG 100枚では、原寸描画データの準備後すぐ次へ進む条件の中央値が、引継ぎ修正の約28.5→22.6msから、さらに約16.6msになりました。一方、最短33ms間隔は約8.0→8.3～8.5msで改善していません。GPU表示時間の測定ではなく、途中の空表示・低解像度表示をなくす要件は引き続き未達です。
