...
4096×2304 JPEG100枚のGPU試験では、閉じる前後のコミット量が約991→459 MiB／1066→531 MiB、ワーキングセットが約666→141 MiB／669→142 MiBへ低下しました。GPU local使用量は約338→338 MiB／338→302 MiBでした。これは全ウィンドウ空状態での自動整理を導入する前の値で、導入後の大画像による統合計測は引き続き必要です。
