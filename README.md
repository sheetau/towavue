...
読み込み済みの画像タブへ戻る時は、原寸画像を再読込せず、拡大率・位置・選択範囲・読書設定とページを復元します。未完了の読み込み／resize処理は復帰時に再開し、閉じたタブの状態は保持しません。開いている画像はタブを閉じるまで保持するため、メモリ使用量はタブ数に応じて増えます。動画・音声の再生session保持とバックグラウンド再生、一覧のscrollなども含む完全な状態復元はまだ未実装です。
...
再生の映像・音声は独立した入力と上限付きqueueで供給します。音声の出力待ちが映像の初期表示や停止中Seekを塞がず、短い映像と長い音声の組合せでも映像を表示できます。音声producerはpipelineごとに一つだけ開始し、映像のhardware fallbackでは再開始しません。映像だけを停止・現在位置から復帰させる内部APIも追加しましたが、通常UIのタブ別バックグラウンド再生への接続はまだ未完了です。
