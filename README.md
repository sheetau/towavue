...
通常起動をウィンドウ管理ホストへ移し、通知の宛先・待機時刻・終了判定をウィンドウ単位に分離しました。非表示の2ウィンドウで、片方の未保存確認を取消／破棄しても、もう片方の動画表示・停止位置を維持することを確認しています。通常操作で同一プロセスへ新しいウィンドウを追加する入口と、タブ状態の移送・共有GPUの一括復旧はまだ未接続です。

ウィンドウ間の再生状態移送に向け、同じD3D11 deviceに複数の描画先を作るruntime基盤を追加しました。非表示の所有ウィンドウ2つで、同じ動画sessionの交互描画・リサイズ・描画先の破棄／再作成と次フレームへの進行を確認しています。通常のタブ分離・結合にはまだ接続しておらず、現在のパスだけを別プロセスで開く動作は変わりません。
