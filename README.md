...
対応範囲のAPNGは、全フレームに編集を適用し、表示時間・繰り返し回数を保って`.png`／`.apng`へ保存できます。PNGテキストの保持・変更・削除も使えます。PREVIOUS方式・単一フレームに加え、独立した表紙画像も同じ編集を適用して保存できるようになりました。表紙はアニメーションのフレームに混ぜません。表紙付き入力は中間PNG列を一枚ずつ生成するため追加の処理・一時ディスク容量が必要です。半透明の重ね合わせの丸め差や、GIF／WebP等のアニメーション保存は引き続き対応中です。

画像の未保存判定では、表示用に処理した画素を比較にも共有し、同じ画像の再生成を省くようにしました。生成4096×2304画像のRelease試験2回では比較中央値が約48→1.1ms、観測した最大コミット量が約207→171MiBでした。表示用画像の生成時間は約46msで変わらず、実UI全体の操作時間を測った値ではありません。

画像編集でも、保存済みの内容と全画素が一致すれば未保存表示を解除します。比較は既存ワーカーで行い、Undo／Redoは残します。寸法を戻しても画素が失われた場合は解除せず、比較が終わるまでは未保存扱いです。通常の画像移動には比較を追加しませんが、編集時の比較負荷は発生します。

区間音量や伸縮を元の内容へ正確に戻した場合も、未保存表示を解除するようにしました。元ファイルの長さが分かるときは、再生・保存に使う区間列を比較し、Undo／Redoは残します。長さが同じでも参照する元区間や音量が違う場合、整数丸めの差が残る場合は未保存のままです。

音声・動画の全体音量や速度を保存済みの値へ戻したとき、未保存の印と保存確認が残らないようにしました。トリムの全体設定も最終値で比較し、Undo／Redoの履歴は残します。区間ごとの編集や画像処理を自動で相殺する変更ではありません。

静止WebPでもJPEGと同じ9項目のXMPメタデータを表示・編集し、WebPへの保存で保持・設定・削除できるようにしました。保存時のメタデータ書換えでは、圧縮画像データを再処理しません。元のEXIF・ICC・未知のXMPの完全保持や、他形式へのメタデータ移送は対象外です。WebPの画像書出しはまだ1枚出力のため、この保存経路ではアニメーションWebPを明示的に拒否します。

全ウィンドウのタブが空になり、空画面の描画が成功してから1秒後に、GPUの内部キャッシュを一度だけ整理するようにしました。他ウィンドウに画像・動画・音声がある場合や、検索・メニュー・開く操作・書出し・復旧の途中は実行しません。直後の開き直しは待機を取り消します。共有GPUの2ウィンドウで表示維持・整理後の再表示と、実イベントループの約1秒の待機を確認済みです。閲覧中のピーク削減や、あらゆる環境での解放量を保証するものではありません。

閉じた後のGPU使用量を追加検証しました。大画像100枚の試験では、通常の終了処理で描画エンジン内の画像テクスチャは8個・253MiBから1個・1MiBへ減っています。GPU使用量の約338MiBは処理完了を待っても残り、検証専用のTrim後に約14MiBへ低下しました。この環境での残量は原寸画像の参照残りではなく、主にDirect3D／driverの再利用領域と考えられます。この結果を基に、上記の全ウィンドウが空の場合だけの整理を導入しました。診断時の値と、製品の統合経路での計測は区別します。

最後の画像タブを閉じると、音声・動画タブが残っていても原寸画像・描画用テクスチャのキャッシュを解放します。復号側の解放は既存ワーカーで行い、閉じる操作は復号完了やキャッシュのロックを待ちません。直後の再openでも解放要求を維持し、別の画像タブが残る場合の表示・キャッシュは保持します。再生セッションの状態・位置・速度・編集履歴と、別ウィンドウへ移した画像は維持します。閲覧中のピークメモリ削減や、全タブ構成の資源認定とは別の改善です。

製品経路の可視ウィンドウで4096×2304 JPEG100枚を順番に表示するRelease試験も2回通過しました。最後のタブを閉じてから約1.02秒で自動整理し、GPU localは両方とも約303→17MiB、現在のコミット量は約959→132MiB／955→131MiBへ低下しました。整理後の開き直しは約44ms／50msで、別途64点の画素一致も確認しています。生成直後の画像・明示的な送り先指定の試験であり、物理キー連打、cold読み込み、Trimなしとの速度比較、閲覧中のピーク削減を確認した結果ではありません。

同じタブ内の画像切替では、新しい原寸が準備できるまで直前の原寸画像を表示します。通常の1枚送りは方向だけを最大256件保持し、各原寸を描画してから次へ進むようにしました。待機中は表示専用とし、編集・保存・画像やパスのコピーを無効にします。直接の飛び先指定、タブ移動、画像順の変更、読み込み失敗などでは残りの送りを取り消します。物理キー入力・可視画面と資源量の妥当性を含む全条件の検証は引き続き必要です。

4096×2304 JPEG100枚に対して送りコマンド100件を一括投入するGPU試験では、全原寸を順番に約2.04秒で描画し、空表示・縮小プレビューは各0件でした。非表示窓でのコマンド境界の試験であり、OSからの物理キー入力を確認した結果ではありません。

100枚のRelease CPU描画計測では、即時連続／33ms間隔とも空表示・縮小プレビューが各0件となり、新しい原寸100枚への到達を確認しました。旧画像の保持は準備完了と別に数えています。実画面のちらつき回数を測った結果ではありません。

原寸の保持と置換は、100・125・200%のGPU画素比較でも確認しました。さらに4096×2304 JPEG100枚を非表示テスト窓でGPU描画し、到達中央値は即時連続約17.2ms／33ms間隔約11.1～11.2ms、空表示・縮小プレビューは各0件でした。別走行では原寸ごとに64点の画素を照合しています。プロセスの最高コミット量は約1.21GiBあり、物理キー連打・他形式・IrfanViewとの比較と資源削減の検討は引き続き必要です。
...
JPEGのメタデータ編集はDate（公開日）とTrackにも対応し、計9項目になりました。公開日は年だけ・年月・年月日や時刻付き表記を受け付け、Trackは整数として検査します。不正な日付や「2/12」の新規設定は拒否しますが、Keepでは既存の表記をそのまま保持できます。撮影日時との同期、Album artistや他形式の完全保持は引き続き未対応です。
...
JPEG も File menu「Metadata export options」から XMP の Title／Artist／Album／Composer／Genre／Date／Track／Comment／Copyright を指定できます。言語別の既存値・作者順を表示し、Keepは対応項目の全値を保持、Setは一つの値へ置換、Removeはその項目の全値を削除します。Dateは公開日で、例は `2026`、`2026-09`、`2026-09-12T08:30+09:00`。時刻やtimezoneを勝手に補完・変換しません。Trackは符号付きも含む十進整数で「2/12」は新規設定できません。Keepでは非標準の日付・Track表記も保持します。JPEG形式で保存してください。設定未使用／全項目Keepの通常保存でも対象9項目を保持します。EXIF／IPTC／JPEGコメントとの同期・Album artist・Extended XMPは未対応で、元の未知／技術XMPはコピーしません。
