...
別ウィンドウからのタブは、最後のタブの右にある空白にもドロップして末尾へ追加できます。空白を掴む通常のウィンドウ移動は維持します。可視操作で未保存編集とフォーカスの移動、Welcomeへ戻した動画の画素一致を確認しました。以前の黒画面という記録は、保存画像の再検証で誤判定と分かり訂正しました。可視分離・遮蔽・混在DPI・全mediaの確認は継続中です。
