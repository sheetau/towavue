...
動画・音声のFile menu「Metadata export options」から、タイトル・アーティストなど10項目をKeep／Set／Removeで指定できます。元ファイルと選択中のstreamの値を非同期で表示し、Apply後のSave／Export as／音声のみ出力に反映します。形式が指定値を保持できなければ、既存の保存先を変更しません。設定は現在のファイル・タブ内だけで保持し、再読込・別ファイル／次曲・タブを閉じると解除します。再生や編集履歴は変更せず、Cancel／Escapeは未適用の入力を破棄します。画像メタデータはまだ未対応です。
