...
動画のFile menu「Export audio only」から、音声だけを別ファイルへ書き出せます。既定は`元の名前-audio.wav`で、WAV／FLAC／MP3／M4A／AAC／Ogg Opus／Opusを選べます。時間範囲・区間削除／伸縮・音量・速度は反映し、映像のcrop／回転／resizeは含めません。再encodeであり圧縮音声の無変換抽出ではありません。元動画の編集履歴・保存済み状態・通常のSave先は変わらず、未保存編集の確認も残ります。音声なし・空の出力・キャンセルでは既存の保存先を保持します。個別metadata書換はまだ未実装です。
