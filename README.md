...
対応範囲のAPNGは、全フレームに編集を適用し、表示時間・繰り返し回数を保って`.png`／`.apng`へ保存できます。PNGテキストの保持・変更・削除も使えます。PREVIOUS合成で消えたはずの画素が戻る表示不具合を修正し、PREVIOUS方式と単一フレームのAPNGも保存可能になりました。独立した表紙画像の保存はまだ明示的に拒否します。半透明の重ね合わせの丸め差や、GIF／WebP等のアニメーション保存は引き続き対応中です。
