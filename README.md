...
閉じた後のGPU使用量を追加検証しました。大画像100枚の試験では、通常の終了処理で描画エンジン内の画像テクスチャは8個・253MiBから1個・1MiBへ減っています。GPU使用量の約338MiBは処理完了を待っても残り、検証専用のTrim後に約14MiBへ低下しました。この環境での残量は原寸画像の参照残りではなく、主にDirect3D／driverの再利用領域と考えられます。Trimは製品へ未導入で、全ウィンドウの動作を妨げない適用条件を引き続き検討します。

最後のタブを閉じると原寸画像・描画用テクスチャのキャッシュを解放するようにしました。復号側の解放は既存ワーカーで行い、閉じる操作は復号完了やキャッシュのロックを待ちません。直後の再openでも解放要求を維持し、別タブが残る場合の表示・キャッシュは保持します。閲覧中のピークメモリ削減や、全タブ構成の資源認定とは別の改善です。

4096×2304 JPEG100枚のGPU試験では、閉じる前後のコミット量が約991→459 MiB／1066→531 MiB、ワーキングセットが約666→141 MiB／669→142 MiBへ低下しました。原寸参照の解放は確認済みですが、GPU local使用量は約338→338 MiB／338→302 MiBで大半が残っており、driver側の保持を含む追加確認が必要です。
