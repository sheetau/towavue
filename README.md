...
画像切替時に進行中の先読みを先に取り消してしまう経路を修正し、移動先への要求で直接引き継ぐようにしました。4096×2304の生成JPEG 100枚では、原寸描画データの準備後すぐ次へ進む条件の中央値が約28.5→22.6msになりました。最短33ms間隔では約8msです。GPU表示時間の測定ではなく、途中の空表示・低解像度表示をなくす要件は引き続き未達です。
