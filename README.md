...
I/Oでtrim端点を指定するとtimelineが開き、保存対象のsource範囲をミリ秒付きで表示します。未指定の開始/終了は先頭/末尾です。逆転・零長・範囲外はその場で拒否し、同じ端点の再指定は履歴を増やしません。現段階では「Export trim」と明示し、再生・Seekはsource全体のままです。範囲内live再生は未実装です。
