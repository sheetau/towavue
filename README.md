...
音声・動画タブを外へドラッグすると、同じプロセスの新しいウィンドウへ再生sessionと未保存編集を移します。停止位置・表示フレーム・選択・プレイリスト設定などを保持し、移動先の作成に失敗した場合は元タブを残します。元の最後のタブを移した後はWelcome画面になります。画像の状態移送と、既存ウィンドウへのドラッグ結合はまだ未接続です。
...
同じD3D11 deviceに複数の描画先を作るruntime基盤を使っています。非表示の所有ウィンドウ2つで、同じ動画sessionの交互描画・リサイズ・描画先の破棄／再作成と次フレームへの進行を確認しています。画像タブの分離とfilmstripからの新ウィンドウは、引き続きパスを別プロセスで開きます。
