...
timelineのシークドラッグを上部の三角マーカーに限定し、CTI線から範囲選択を始められるようにしました。選択の左右端をドラッグして範囲を調整でき、音量線は白50%と上下調整カーソルになります。seekbarにはホバー位置まで白25%の進捗を追加しました。入力・描画の自動回帰は確認済みですが、今回の実入力・実画面は未確認です。timeline背景・選択塗りの追記は引き続き作業中です。
