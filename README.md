...
選択範囲の角をドラッグすると二辺を同時に変更でき、Shiftを押すと対角を固定したまま比率を保ちます。画像の選択範囲内を右ドラッグすると、画像ではなく選択範囲だけを同じサイズで移動します。画像端で停止し、取消すると元の選択へ戻ります。外部クリックでの解除と、従来のcrop previewから通常ズームへの変更は引き続き未対応です。
