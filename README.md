...
| `src/site/media.js` | Generated media metadata, fictional audio tracks, and clock formatting |
| `src/site/media-manifest.json` | Photo/video metadata generated by `scripts/prepare-media.py` |
...
The Japanese page includes the reading `トワビュー` only in its `keywords` meta tag and the application's structured-data `alternateName`; visible copy, titles, descriptions, and image alt text use `towavue`. Google does not use the keywords meta tag for indexing or ranking. Open Graph and Twitter cards use an unchanged copy of the supplied `images/image2.png` at `public/media/image2.png` (997 × 622); the media preparation script preserves this copy on regeneration.
