...
動画の細いシークバーを横にドラッグすると、一時停止してメイン映像に低解像度プレビューを表示します。離した時に一度だけSeekし、元が再生中なら再開します（終端は停止）。Escape・フォーカス喪失ではSeekせず取り消します。切り抜き・回転・反転・リサイズと表示倍率をプレビューにも反映しますが、粗い時刻サンプルなので正確なフレーム確認は離した後に行います。
