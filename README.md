...
通常起動をウィンドウ管理ホストへ移し、通知の宛先・待機時刻・終了判定をウィンドウ単位に分離しました。非表示の2ウィンドウで、片方の未保存確認を取消／破棄しても、もう片方の動画表示・停止位置を維持することを確認しています。通常操作で同一プロセスへ新しいウィンドウを追加する入口と、タブ状態の移送はまだ未接続です。
