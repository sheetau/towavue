...
画像の原寸データが完成していても、完了通知より描画が先に来ると空表示／縮小プレビューを描く経路を修正しました。描画開始時に完了済みの結果だけを取り込みます。ただし100枚のCPU描画計測では、切替直後の空表示がすべての切替で残っています。原寸準備の中央値は即時連続約17.0ms／33ms間隔約7.7msで、復号待ちを含むシームレス切替はまだ未達です。
