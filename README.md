...
対応範囲のAPNGは、全フレームに編集を適用し、表示時間・繰り返し回数を保って`.png`／`.apng`へ保存できるようになりました。PNGテキストの保持・変更・削除も使えます。単一フレーム、独立した表紙画像、PREVIOUS合成方式はまだ保存できず、明示的に拒否します。半透明の重ね合わせには表示側と保存側の丸め差が残り、GIF／WebP等のアニメーション保存も引き続き対応中です。
