...
一度開いた静止画は、最大8枚・256 MiBのdecode cacheをwindow内で共有し、戻る操作での再decodeを省きます。fileのサイズ・更新時刻が変われば読み直します。animationと容量を超える画像はcacheせず、先読み・画質変更は行いません。cacheはwindowを閉じるまで保持するため、表示中以外の画像にもメモリを使います。
