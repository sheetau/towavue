...
大きなJPEGを初めて開く場合、原寸より先に240×160以内の低解像度プレビューを用意します。EXIFの向きと元寸法を保ち、原寸の準備ができたら置き換えます。生成6000×4000画像のRelease測定では、プレビュー取得が約13ms、原寸完了が約55msでした（プレビューなしの原寸は約42ms）。黒い待ち時間を減らすため総処理時間が増える方式です。小画像・予算超過・対応できない入力は通常復号へ戻し、他の静止画形式の初回プレビューは未完です。
