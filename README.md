...
同じタブ内の画像切替では、新しい原寸が準備できるまで直前の原寸画像を表示するようにしました。待機中は表示専用とし、編集・保存・画像やパスのコピーを無効にします。連打は最新要求を優先し、失敗・別タブへの移動・終了時には保持画像を解放します。100・125・200%の自動描画回帰は確認済みですが、可視画面での連打時の全画像到達・資源量の妥当性を含むシームレス切替の全条件はまだ未達です。
