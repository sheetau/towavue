    ...
    fn options(&self) -> Result<MetadataExportOptions, String> {
        if self.kind == MediaKind::Image {
            if self.image_format().is_none() {
                return Err(
                    "Image metadata requires PNG, JPEG or static WebP input with the same output format.".into(),
                );
            }
            match &self.current {
                Some(Ok(_)) => {}
        ...
        let modal = egui::Modal::new(egui::Id::new(("metadata-export-options", self.token))).show(context, |ui| {
            ...
            egui::ScrollArea::vertical().max_height((context.content_rect().height() - 128.0).max(20.0)).min_scrolled_height(20.0).show(ui, |ui| {
                ...
                if image_format == Some(ImageMetadataFormat::Png) {
                    ...
                    ui.label("Supported APNG saves retain all frames, delays and loop count (1 to 65536 frames), including PREVIOUS disposal. A separate default poster receives the same edits and stays outside the animation. Alpha-over rounding can differ from display. Other animation formats are separate capabilities.");
                    ...
                    ui.label("Static WebP input and WebP output only. Applies to the next Save or Export as for this tab's current file. Original file, displayed pixels and edit history stay unchanged. Animated WebP is rejected.");
                    ...
                    ui.label("Image metadata currently supports PNG, JPEG and static WebP only, with the same input/output format. Other image formats cannot apply metadata options.");
                } else {
