        ...
        let modal = egui::Modal::new(egui::Id::new(("metadata-export-options", self.token))).show(context, |ui| {
            ...
            egui::ScrollArea::vertical().max_height((context.content_rect().height() - 128.0).max(20.0)).min_scrolled_height(20.0).show(ui, |ui| {
                ...
                if image_format == Some(ImageMetadataFormat::Png) {
                    ...
                    ui.label("Only these 9 XMP fields are edited. EXIF, IPTC and JPEG comments (COM) are not synchronized. Same-format saves without image edits retain unselected XMP; all-Keep preserves the original packet. Image edits or format conversion copy only the supported XMP fields. Keep preserves languages, author order and existing noncanonical Date/Track values; new values must match the displayed types.");
