        ...
        let modal = egui::Modal::new(egui::Id::new(("metadata-export-options", self.token))).show(context, |ui| {
            ...
            egui::ScrollArea::vertical().max_height((context.content_rect().height() - 128.0).max(20.0)).min_scrolled_height(20.0).show(ui, |ui| {
                ...
                if image_format == Some(ImageMetadataFormat::Png) {
                    ...
                    ui.label("Supported APNG saves retain all frames, delays and loop count (1 to 65536 frames), including PREVIOUS disposal. A separate default poster is not yet supported for export. Alpha-over rounding can differ from display. Other animation formats are separate capabilities.");
