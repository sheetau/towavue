        ...
        let modal = egui::Modal::new(egui::Id::new(("metadata-export-options", self.token))).show(context, |ui| {
            ...
            egui::ScrollArea::vertical().max_height((context.content_rect().height() - 128.0).max(20.0)).min_scrolled_height(20.0).show_styled(ui, |ui| {
                ...
                if image_format == Some(ImageMetadataFormat::Png) {
                    ...
                    ui.label("JPEG input supports JPEG or WebP output. Applies to the next Save or Export as for this tab's current file. Applying options does not write the file. Save replaces the source; Export as writes a separate file.");
                    ...
                    ui.label("WebP input supports WebP output, or JPEG output for static images. Applies to the next Save or Export as for this tab's current file. Applying options does not write the file. Save replaces the source; Export as writes a separate file. Animated WebP retains all frames, exact timing and loops. With image edits, lossless full-canvas snapshots share display's compositing and edits.");
                    ...
                    ui.label("Applies to the next Save, Export as and Export audio only for this tab's current file. Applying options does not write the file or change playback. Save replaces the source; exports write separate files.");
