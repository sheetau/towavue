        ...
        let modal = egui::Modal::new(egui::Id::new(("metadata-export-options", self.token))).show(context, |ui| {
            ...
            egui::ScrollArea::vertical().max_height((context.content_rect().height() - 128.0).max(20.0)).min_scrolled_height(20.0).show_styled(ui, |ui| {
                ...
                if image_format == Some(ImageMetadataFormat::Png) {
                    ...
                    ui.label("WebP input supports WebP output, or JPEG output for static images. Applies to the next Save or Export as for this tab's current file. Original file, displayed pixels and edit history stay unchanged. Animated WebP retains all frames, exact timing and loops. With image edits, lossless full-canvas snapshots share display's compositing and edits.");
