...
fn metadata_ui_reads_source_blocks_unsupported_or_failed_reads_and_explains_scope(
    ...
    let expected = if matches!(extension, "png" | "apng") {
        ...
        vec![
            ...
            "Same-format saves without image edits retain unselected XMP",
            "all-Keep preserves the original packet",
            "Image edits or format conversion retain these 9 fields plus XMP rights properties",
            "Owner, UsageTerms, WebStatement, Marked",
            "other XMP properties are omitted",
            "and keywords (dc:subject)",
            "contributor/publisher credits (dc:contributor, dc:publisher)",
            "Set/Remove does not change those separate rights properties, keywords or credits",
...
#[test]
fn avif_to_apng_save_as_and_resave_preserve_animation_and_history() {
    let Some(root) = crate::tests::isolated_test_root(
        "metadata_export::tests::image::avif_to_apng_save_as_and_resave_preserve_animation_and_history",
    ) else {
        return;
    };
    animation_conversion_lifecycle(&root, "avif", "apng");
}
...
fn webp_to_avif_save_as_and_resave_preserve_animation_and_history() {
    ...
    animation_conversion_lifecycle(&root, "webp", "avif");
}

#[test]
fn avif_to_webp_save_as_and_resave_preserve_animation_and_history() {
    let Some(root) = crate::tests::isolated_test_root(
        "metadata_export::tests::image::avif_to_webp_save_as_and_resave_preserve_animation_and_history",
    ) else {
        return;
    };
    animation_conversion_lifecycle(&root, "avif", "webp");
}

#[test]
fn avif_to_gif_save_as_and_resave_preserve_animation_and_history() {
    let Some(root) = crate::tests::isolated_test_root(
        "metadata_export::tests::image::avif_to_gif_save_as_and_resave_preserve_animation_and_history",
    ) else {
        return;
    };
    animation_conversion_lifecycle(&root, "avif", "gif");
}
