...
fn png_fixture(root: &Path) -> PathBuf {
...
fn values(path: &Path) -> Vec<MetadataSourceValue> {
    towavue_runtime_windows::read_export_metadata(path, MediaKind::Image).expect("PNG values")
}

#[test]
fn png_metadata_ui_reads_source_blocks_unsupported_or_failed_reads_and_explains_scope() {
    let Some(root) = crate::tests::isolated_test_root(
        "metadata_export::tests::png::png_metadata_ui_reads_source_blocks_unsupported_or_failed_reads_and_explains_scope",
    ) else {
        return;
    };
    let source = png_fixture(&root);
...
fn png_metadata_save_resave_all_keep_remove_format_failure_guard_and_source_lifecycle() {
    let Some(root) = crate::tests::isolated_test_root(
        "metadata_export::tests::png::png_metadata_save_resave_all_keep_remove_format_failure_guard_and_source_lifecycle",
    ) else {
        return;
    };
    let source = png_fixture(&root);
    ...
    let bad = root.join("unsupported.jpg");
    ...
    assert!(
        app.export_error
            ...
            .contains("PNG input and PNG output")
    );
    ...
    std::fs::copy(&source, root.join("other.png")).expect("other image");
    app.tabs.activate(tab);
    app.displayed_tab = Some(tab);
    app.activate_tab(other);
    assert_eq!(app.path.as_deref(), Some(root.join("other.png").as_path()));
    assert_eq!(app.metadata_export_settings.get(&tab), Some(&setting()));
