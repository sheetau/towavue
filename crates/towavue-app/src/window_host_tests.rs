...
fn new_tabs_inherit_last_listening_volume_across_windows_without_rewriting_existing_tabs() {
    ...
    if let Ok(phase) = std::env::var("TOWAVUE_VOLUME_RESTART_TRIAL") {
        let mut host = WindowHost::new(None, None).expect("restarted host");
        let app = host.windows.values_mut().next().expect("app");
        app.open_external(paths[0].clone(), true);
        if phase == "muted" {
            assert_eq!(app.playback_volume(), 0.0, "mute survives process exit");
            app.toggle_playback_mute();
            assert_eq!(
                app.playback_volume(),
                1.2,
                "unmute restores the saved positive level"
            );
            app.set_playback_volume(0.6789);
        } else {
            assert_eq!(phase, "level");
            assert_eq!(
                app.playback_volume(),
                0.6789,
                "new process reads the last adjustment"
            );
        }
    ...
    eprintln!(
        "PASS listening volume: shared windows, old tabs preserved, muted/positive values restored across two process restarts, export gain unchanged"
    );
}
