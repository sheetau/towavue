...
fn orient_still(path: &Path, angle: u8, mirror: Option<u8>, alpha: bool) {
    ...
    let count = boxes(&mut file, ipco.start, ipco.end, &cancel)
        .expect("fixture")
        .len();
    properties.push((b"irot", angle));
    if let Some(axis) = mirror {
        properties.push((b"imir", axis));
    }
    fn rewrite(
        file: &mut fs::File,
        item: BoxRange,
        ...
        match &item.kind {
            b"meta" | b"iprp" => {
                ...
                .expect("fixture")
                {
                    data.extend(rewrite(file, child, props, count, alpha));
                }
            }
            b"ipco" => {
    ...
    let replacement = rewrite(&mut file, meta, &properties, count, alpha);
