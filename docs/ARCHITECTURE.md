...
- AVIF one-frame sequence export and complex export edit lists currently fail explicitly. Static AVIF keeps its existing export path, without auxiliary-alpha export support. AVIF metadata editing/full color fidelity are unsupported. Source/output geometry checks bound individual RGBA canvases, not the whole process: the FFmpeg AVIF muxer buffers encoded media in memory. Long-animation cost and end-to-end premultiplied/oriented/HDR coverage remain unverified.
