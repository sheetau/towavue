...
appのmedia読み込み番号をopen（失敗を含む）と最後のtab closeごとに更新し、再生callback・duration・waveform・hover thumbnailの結果へ付ける。現在の番号だけを受理し、同じpathへの再openでも旧結果を破棄する。再生はその上でruntimeのaccepts_eventでstream世代を照合する。音声／全decode完了はSeek/recovery世代、映像の準備・decode path・device fault／VideoFailedは映像worker停止時にも進む独立世代を使う。全decode完了通知はwakeとして扱い、現在のruntime完了状態を再確認する。映像だけの復帰前にqueueへ入った完了／故障を、新しい映像へ誤適用しない。
...
PlaybackSession::set_video_visibleは非表示時に映像queue／frameを解放して映像workerだけを取消・joinし、表示復帰時に呼出側の現在source位置から映像入力を開き直す。音声／sessionの世代、pause・volume・rate・rangeと音声workerは保持する。取消はpacket/output境界で確認し、receiverを先にdropして満杯のbounded producerを解放する。実行中のFFmpeg／storage callまで即時中断する保証はない。Seekとdevice交換は従来通り全pipelineを停止・再開し、hidden指定は維持する。このAPIはU07のruntime準備で、通常UIのtab切替にはまだ接続しない。全tabのsession／clock保持、hidden中の音声drainと無音動画の終端判定、復帰時の表示待機、device交換時の全session停止／復旧は次のapp接続で検証する。映像入力も完全に保持して再openをなくす最適化は、この映像worker再開APIだけでは達成しない。
