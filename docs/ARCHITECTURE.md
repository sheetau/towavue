...
PlaybackSession::set_video_visibleは非表示時に映像queue／frameを解放して映像workerだけを取消・joinし、表示復帰時に呼出側の現在source位置から映像入力を開き直す。音声／sessionの世代、pause・volume・rate・rangeと音声workerは保持する。取消はpacket/output境界で確認し、receiverを先にdropして満杯のbounded producerを解放する。実行中のFFmpeg／storage callまで即時中断する保証はない。Seekとdevice交換は従来通り全pipelineを停止・再開し、hidden指定は維持する。U07で通常UIのtab保持へ接続し、終端が既知の非active動画でこのAPIを使う。映像入力も完全に保持して再openをなくす最適化や、復帰時の表示待機短縮は未完である。
