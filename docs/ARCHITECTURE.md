...
logo menuをEscapeで閉じ、commandを選んでいない場合はlogo buttonへfocusを戻す。親menu・submenu共通で、次のEnter/Spaceから再openできるようにする。背景click・command実行・通常再描画ではこのfocus復帰を要求しない。
...
command paletteを開く直前のfocusを一件だけ保持し、Escapeによる取消時に復帰を要求する。command実行や外部dropで閉じる場合は破棄し、新しい操作先へ古いfocusを戻さない。復帰先widgetの有効性・生存は既存eguiのfocus登録で判定する。これはpalette取消の契約であり、全modal間のfocus stackではない。
