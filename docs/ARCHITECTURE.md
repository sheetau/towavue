...
- Decode static images and animations to safe RGBA frames with delays. Image-crate decoders apply EXIF orientation; AVIF orientation coverage remains incomplete. AVIF uses paired FFmpeg color/gray decoders; animated PNG uses the runtime compositor.
...
- PNG/APNG retains ten supported text fields; JPEG and WebP retain nine supported XMP fields. Preserve supported language/author structures and existing noncanonical date/track values under Keep; Set validates typed values. Unknown XMP, EXIF/IPTC/COM/ICC synchronization and cross-format transfer are not implied.
...
- WebP export does not retain the whole decoded sequence or stage a PNG sequence. Full snapshots/lossless encoding can increase file size and work; long-animation performance remains unmeasured. Canvas RGBA and encoded ANMF payloads are each bounded to 512 MiB and output frames to 16384 per dimension, with the RIFF file limit enforced; these are not a whole-process memory cap. Per-frame codec/filter calls are not interruptible. Cross-format animation conversion is rejected, not flattened; static WebP conversion remains available. Source ICC, EXIF and unknown metadata retention are separate limitations.
- AVIF sequence export selects timed AV1 tracks by ID/time base/media duration, validates the auxiliary alpha type/link and aligned geometry/timing, and reads repetition from elst/tkhd. Supported 2–65536-frame sequences use RGBA8 edits, lossless RGB AV1 and a separate monochrome alpha track when needed. Validate exact rational frame times/durations, loop policy, track count and dimensions before publication. An absent repetition declaration remains absent. Shared staging/cancellation/source-identity guards apply; animated cross-format conversion fails rather than flattening.
