...
- Use the selected FFmpeg best streams consistently for playback, preview, waveform, and export. AVIF sequence export explicitly selects timed color/auxiliary-alpha tracks: primary still items may have the same IDs and are not animation sources.
...
- AVIF one-frame sequences and complex edit lists currently fail explicitly. Static AVIF keeps its existing export path. AVIF metadata editing/full color fidelity are unsupported; auxiliary-alpha display is still separate work. Source/output geometry checks bound individual RGBA canvases, not the whole process: the FFmpeg AVIF muxer buffers encoded media in memory. Long-animation cost and premultiplied/oriented/HDR coverage remain unverified.
