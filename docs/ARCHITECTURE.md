...
- GIF→GIF export preserves 1–65536 frames, exact centisecond delays (including zero), and absent/infinite/finite repeat settings. A bounded indexed-frame scan validates source pixels/LZW, geometry, and controls; FFmpeg alone can silently tolerate damaged GIFs even with `-xerror`. Each edited frame gets its own palette, without forcing a single output frame; staging rewrites palette indices to restore exact controls and validates the result before publication. No entire decoded animation is retained by this scan/rewrite; the 512 MiB canvas/decoder limits are not a process-memory cap. Extra decode/encoding work, color quantization, and binary transparency remain costs/limitations. GIF metadata editing and arbitrary extension/ICC retention are not supported. Animated GIF conversion to other formats is rejected rather than silently flattened; static GIF conversion remains available.
