...
After checkpoint 7c5d073, the application gained the [Help/palette material entry](DEVELOPMENT.md). The fixed inputs below still describe the earlier 03125262 executable and are intentionally unchanged. They must be rebound to the final application/source snapshot before release; do not label the new 195af870 UI build with this older binding. The installed-app path contract is `licenses/START-HERE.html`, selected in Explorer. It does not decide that every source archive must be installed locally.
...
The Browser skill's selected browser blocked file-URL navigation. No alternative browser or transport was used to bypass that restriction. Link/file validation is not rendering evidence; actual page rendering remains unverified. The later application-menu/palette entry is tested separately with an explicitly labeled path-selection fixture; it does not execute HTML or establish the final bundle's source correctness.

The specifically named native notice/data collection work is now represented in the local catalog. Keep documented historical reproduction/other-target limits, but do not reopen all permissive source as a universal rebuild gate. The final candidate needs refreshed app/source binding, same-release delivery and the guide's actual installer placement. VC prerequisite lifecycle, assisted Setup.exe, supported-Windows installation/update/removal, final runtime quality and owner acceptance remain separate. Publication/signing and any distribution adoption are not implied by collection success.
