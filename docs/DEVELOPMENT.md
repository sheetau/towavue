...
Set `TOWAVUE_COLOR_INTEGER_TRIAL=1` for current-integer/historical-lookup/egui/egui/historical-lookup/current-integer batches. This compares production's exact integer premultiplication against the former per-pixel lookup implementation, retaining the same opaque-row shortcut. The example's ordinary Release tests cover every component/alpha pair and generated row boundaries; full image equality is outside timed conversions.
