...
- Hold a per-user fixture operation lease across each install/uninstall section, using the same helper and native acquisition protocol as local application Setup. A competitor exits 4 without target changes. The fixture identity is separate from real application registration and creates no registry values. PowerShell is used only to derive the kernel-object name; no shared runtime or application is installed.
...
Private NSIS copies insert event-controlled gates immediately after acquiring the lease in both sections. While each real NSIS process is paused, a second installer, uninstaller and PowerShell lease acquisition are refused and all target bytes remain unchanged. Normal release completes document installation/removal; a terminated pre-write installer releases its handle and a fresh install/removal retry succeeds. The production source has no pause/termination switch. Both 32-bit and 64-bit Windows PowerShell drive these tests against the native 32-bit NSIS binaries. This is not old-uninstaller/update compatibility, hostile-process exclusion, power-loss recovery or actual application lifecycle proof.

The process observer logs a still-live fixture at 15 seconds and waits another 45 seconds on that same cached handle; it does not restart the process. A private 30-second gate verifies this continued observation and subsequent normal install/removal. CI previously stopped observing a live fixture at 15 seconds; the original process's eventual result was not captured, so a cold start or a different runtime failure must not be asserted from that timeout alone. Longer unresolved waits still fail with PID/start time/executable/arguments for diagnosis.

For application/source/notices, prerequisite consent and per-user registration, see [LOCAL_SETUP](LOCAL_SETUP.md), [VC_REDIST](VC_REDIST.md) and [CANDIDATE_MATERIALS](CANDIDATE_MATERIALS.md). Production assembly is covered by [RELEASING](RELEASING.md). This isolated fixture remains a regression tool; it does not qualify a release or replace installed-product verification.
