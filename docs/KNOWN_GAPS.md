...
2026-09-10 U08 filmstrip: 外へのprimary dragから、元ファイルを別windowへ開く入口を追加した。これは元tabを閉じないpath参照の新規Openで、未保存編集／再生状態の移送ではない。起動失敗・古いsnapshot／所属外path・overlayを拒否し、元tabを保持する。単一path引数と成功／失敗のapp状態、合成gestureとhidden GPUのpreview描画／取消を確認するが、今回の試験では実子processを起動していない。spawn成功だけでは子windowの描画／読込成功を保証しない。通常window／物理capture・混在DPI、window間結合／状態移送は未完。

2026-09-10 U08 drag: bar内の操作はtab本体の追従・隣接tabの即時移動・端の横scrollへ対応した。表示だけを投影し、TabSetの確定はrelease一回。window内bar外へのdropとEscape等は取消、外へのdropは従来のguard付きpath-only別process起動のまま。連続したwindow間結合／状態移送とfilmstrip分離は未実装。3幅×3密度・UIA座標とhidden native GPUの描画は確認するが、通常windowの物理drag／mixed-DPI・実CursorLeft/captureの最終認定は残す。
...
2026-09-10: V03の単一source区間modelと保存backendは削除による前後連結、Keep、部分音量／伸縮、履歴からの復元を扱う。rubber-band／部分stretchは接続済みで、範囲再生も接続し、最終実操作／保存再openの総合検証が残る。有限durationが必要で、全範囲削除はUndo可能な空timelineとして保持し、空mediaのexportは拒否する。動画のframe量子化、atempoのtail補正、長い履歴の負荷・継ぎ目の音質を含む最終確認は残る。

2026-09-10: 動画の上drag展開／専用timeline button除去／timeline thumbnail廃止（V01）と音声のfullscreenを含む常時timeline（A02表示部分）を実装した。横Seekの初動固定、取消／複数pass、位置・履歴保持を回帰と通常releaseで確認。時間範囲選択・rubber-band・複数区間／部分音量・速度は接続したが、範囲再生も接続したが、最終監査が残るためV03全体の完了とは扱わない。先行thumbnail sheet（V02）も未完。
...
通常UIのtab切替で動画／音声session・clock・viewを保持し、非activeでも音声再生と終端処理を続ける。既知の終端がある動画は映像decodeを止め、未知ならclockに合わせて有界queueを処理する。復帰時は保持した映像入力を再利用し、最後の表示frameがあれば直ちに描く。現在位置の新frameへの置換にはdecoder再構築／Seek／decode待機が残る。保持hardware frameがsurface poolをpinする場合のメモリ削減は未完。全sessionの同一deviceへの再接続を確認したが、実device removal／endpoint欠落／復旧失敗からRetryまでの全組合せ、全tabのresource予算・全focus状態の保持は未完。実WASAPI試験はmute状態のworker identity／source clock／drain、通常windowでは無音WAVの背景再生位置を確認し、聴感やloopbackでの無音区間測定ではない。
...
| 自由回転 | 画像の角度dialog／slider／Alt保持drag／配置preview／非同期確定と、動画の角度UI／Alt保持drag／同一device GPU preview／保存を実装。通常window／全素材画質／性能は未完 |
| Clipboard copy | 未実装 |
...
| Audio-only export、normalize、stereo/mono変換 | File menuから7形式へ音声のみ出力、normalize／Mono・Stereo設定と通常Save連携を実装。設定はtab内の現在sourceだけで保持し、再起動を越えない。全codec／通常window品質は未認定 |
