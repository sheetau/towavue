...
- 静止画の再訪は最大8件・256 MiBのdecode cacheで高速化し、RGBAをappと共有する。file size/更新時刻の変更とmetadata失敗で失効する。同じdecode identityのtextureも最大8件・RGBA相当256 MiBで再利用し、graphics復旧時に消す。2026-09-09には移動方向の隣一件を別workerで先読みし、このdecode cacheを共用するようにした。6000×6000 PNGを600ms間隔で初めて開くtitle完了の中央値は220.113→31.840ms。見開きも読込済みページから逐次表示するようにした。未読込ページの寸法は仮置きで、実寸取得時には再配置される。低解像度RGBAも別の64件／16 MiB cacheで共用し、原寸画像からfilmstrip／画像seek用に登録するが、メイン画面の段階表示には未利用。初回起動／cold storage、画素変換・GPU upload、連打中の黒いloading、tab hover／recentと未訪問preview先行生成は残る。これらのcache上限はprocess全体のメモリ上限ではなく、title完了は物理表示遅延でもない。
