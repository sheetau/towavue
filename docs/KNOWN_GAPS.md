...
| accessibility | AccessKit adapterを初回表示前に接続し、要求時のtree生成/action配送を実装。Windows UI AutomationでWelcome、menu、paletteの入力/実行と保存確認中の背景Invoke拒否を確認。保存関連3 modalのIsModalと子buttonをnativeで取得できた試行もある | 保存確認のClose window/Cancel反復だけでも3回目にUIA取得が停止する。意味情報追加前相当の比較も同様で原因未確定。headless 5往復成功はWindows経路の修復を証明しない。screen reader、読み順/focus、timeline・selection・playlist等も未完了 |
