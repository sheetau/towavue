...
U07の画像側では読み込み済みの画素・texture・zoom／pan／selection・読書設定／ページをtabごとに保持する。開いた画像のsnapshotはcloseまでpinするので、decode／texture cacheの256 MiBはprocess全体の上限ではない。未完了decodeは復帰時に再要求し、未完了resizeも元Arcから再処理する。一覧のscroll／focusの完全保持は未完。動画・音声sessionの保持と背景再生は後続checkpointで接続した。device世代の違う復帰画像は再upload対象とするが、この保持経路の実GPU removal／混在DPI監査は未実施である。

通常UIのtab切替で動画／音声session・clock・viewを保持し、非activeでも音声再生と終端処理を続ける。既知の終端がある動画は映像decodeを止め、未知ならclockに合わせて有界queueを処理する。復帰時の映像入力は再openし、最初のframe待機は残る。全sessionの同一deviceへの再接続を確認したが、実device removal／endpoint欠落／復旧失敗からRetryまでの全組合せ、全tabのresource予算・一覧scrollの完全保持は未完。実WASAPI試験はmute状態のworker identity／source clock／drain、通常windowでは無音WAVの背景再生位置を確認し、聴感やloopbackでの無音区間測定ではない。
