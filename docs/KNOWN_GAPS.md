...
2026-09-10: 自由回転は画像基盤の段階で、UIのmenu／角度入力／hold-dragは未接続。検証済みcanvas寸法と0.1度角度を編集履歴へ持ち、非同期のRGBA処理とPNG exportが同じfilter列を使う。任意角度はGBRAP8でalphaを乗算して補間するため、再サンプリングによる画質変化はあり、無損失なのは90度単位の専用経路だけ。大画像／多数animation frameの性能、alpha非対応形式や動画への接続はまだ認定していない。動画・音声への画像自由回転exportは明示拒否する。
