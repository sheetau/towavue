...
- 静止画の再訪は最大8件・256 MiBのworker内decode cacheで高速化し、RGBAをappと共有する。file size/更新時刻の変更とmetadata失敗で失効する。続いて同じdecode identityのtextureも最大8件・RGBA相当256 MiBで再利用し、graphics復旧時に消す。6000×6000 PNGと小画像の5往復で、大画像のtitle完了までの中央値は220.651→49.705→16.500ms。初回decode/変換/upload、cold storage、先読みは別課題で、両cacheを合わせたprocess全体のメモリ上限でも物理表示遅延の測定でもない。
...
| accessibility | AccessKitのtree/action配送、Welcome/menu/palette、保存確認中の背景拒否、再生・画像位置・trim端点の値操作を確認。trimのfocus/Undo・範囲拒否も通常windowで検証。Shell STA停止を修正。tabとplaylist/filmstripの対象ID・名前・Invoke・path説明を検証。playlistの画面外行focus移動/選曲、filmstripの連続Tab時の現在項目focus・Escapeと未保存Cancelも確認 | screen reader、全体の読み順/focus、selection、画面外項目へ支援技術だけで到達する全操作は未完了。一覧のkeyboard試験もscreen readerの全操作を証明しない。native試験を支援技術・実環境の横断matrixの代替にはしない |
