...
For a bound transaction, perform the registration transition after the other files have reached their target state and immediately before publishing the executable, in both directions. A registry write failure during file changes leaves the executable retired. The same journal/digest can then Rollback both files and a known partial registry transition. Unknown registry values stop before files change. A file-unchanged operation can adjust the registry without retiring an already consistent executable. Results explicitly distinguish `files_and_registration_applied`/`files_and_registration_rolled_back` from the legacy `payload_applied_registration_pending`/`payload_rolled_back` cases. A partially applied update must be rolled back before preparing a fresh attempt; this is not forward-resume support.
...
This entry point holds a user/install-bound named mutex across preparation, publication and recovery, including across process bitness. Abandonment does not erase the durable pointer. The new registration helper refuses VerifyRemoval/Remove whenever a pending value exists, including malformed values. Neither mechanism retrofits the previously compiled uninstaller or protects the whole interval after NSIS removal verification. Full installer/uninstaller exclusion, retiring/protecting the old uninstaller, publication order and actual recovery UI remain required before enabling occupied-directory Setup.
