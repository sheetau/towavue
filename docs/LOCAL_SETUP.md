...
`scripts/setup-update-transaction.ps1` is a dot-sourced, **not yet Setup-connected** primitive. `New-TowavueUpdateTransaction` accepts the planner's three inputs plus a caller-supplied new uninstaller, which must be generated/verified by the eventual Setup. It creates independent old/new copies in a fresh same-volume sibling directory and writes an immutable journal last, returning `TransactionDirectory` and `JournalSha256`. The future caller must durably retain that digest before calling `Invoke-TowavueUpdateTransaction -Mode Apply`; the digest is a corruption check, not authentication against the same user. Preparation does not change installed files.
