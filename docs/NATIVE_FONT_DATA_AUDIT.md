...
The external libunibreak data inputs are retained in ignored audit storage, with fixed URLs and hashes in the evidence; they are not added to this source kit. Fontconfig's actual data and HarfBuzz/libunibreak generated tables and code notices are inside the preserved archives and selected documents. The prior PCRE2/libxml2 external inputs likewise still need joining to the final data-material delivery. The new Unicode notice is included in supplement v13, but older fixed kits and catalog v5 remain unchanged.

Next, finish the named embedded-code/NOTICE cases in [the package review](NATIVE_NOTICE_REVIEW.md) and connect final notices/source access. Do not turn this bounded check into a universal rebuild requirement for permissive libraries. Installer, runtime quality, supported-Windows lifecycle and owner acceptance gates remain open.
