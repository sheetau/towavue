...
1. The named Unicode references are now identified: [fontconfig/HarfBuzz/libunibreak review](NATIVE_FONT_DATA_AUDIT.md) retains actual 17.0 and mixed 15.0/15.1 evidence, Microsoft's separate USE MIT text and source-test notices; all six libunibreak tables reproduce. [PCRE2/libxml2 reproduction](NATIVE_UNICODE_TABLE_AUDIT.md) establishes exact 17.0.0/4.0.1 inputs. Supplement v13 includes the current Unicode notice alongside FriBidi's unchanged pinned notice. External data/generator material delivery is still pending; none of these source comparisons is a DLL rebuild or exhaustive HarfBuzz generator-environment check.
2. [libjxl/libopenmpt embedded-code review](NATIVE_CODEC_EMBEDDED_AUDIT.md) now retains the actual xorshift MIT, Vector Class Apache and TinyFFT BSD-2-Clause reference notices, original adaptations and build consumers. The 42-owner supplement distinguishes disabled dependencies and source extras from library code. Continue specifically named libpng/libwebp and external OpenCL Headers scope; do not turn this into an unbounded audit of every permissive source.
