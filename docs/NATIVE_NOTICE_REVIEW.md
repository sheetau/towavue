...
2. Continue the named attribution/embedded-code gaps above, prioritizing libjxl and libopenmpt after the completed OpenSSL/OpenCL/libva author/NOTICE check. libjxl's actual recipe disables SKCMS/SJPEG and uses system Brotli/Highway/LCMS; do not demand sources for disabled implementations or treat that configuration alone as a complete library inventory. Keep the explicitly unreviewed external OpenCL Headers scope separate.
