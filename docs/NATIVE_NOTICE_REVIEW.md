...
Both archives and five patch inputs match the build-record-bound recipe checksums. Archive inventories contain 505/4524 safe regular-file/directory entries. The [34-owner supplement](native-source-supplements.json) retains 23 selected originals for these two owners, source archives and all five patches. Source signatures, patch application, rebuild and final binary selection are not newly verified here.
