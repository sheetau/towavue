...
[OpenSSL／OpenCL／libvaの表示確認](NATIVE_PLATFORM_NOTICE_AUDIT.md)では、root本文だけでは分からない作者と別条件を補完した。3原本archiveと6 patch/source inputsはrecipe hashへ対応し、62選択fileを40-owner supplementへ保持する。原文のないNOTICEやcopyright行は創作せず、OpenSSLへ追加されるCC0、生成toolとDLLの違い、libvaのcore／Windowsの両sourceを区別する。[libjxl／libopenmpt](NATIVE_CODEC_EMBEDDED_AUDIT.md)の2 sourceと32選択file、xorshift／Vector Class／TinyFFTの原文も42-owner supplementへ追加した。参照元noticeの固定commitを過去の取り込みcode版と同一視しない。続く[libpng／libwebp／OpenCL Headers確認](NATIVE_IMAGE_HEADER_NOTICE_AUDIT.md)で、PNGの別条件、WebPのPATENTS／AUTHORSと実header原文を44-owner supplement／10-input API-header kitへ保持した。次は最終source／notice提供であり、配布・品質gateは継続する。
