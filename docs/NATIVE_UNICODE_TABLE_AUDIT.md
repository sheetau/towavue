...
The new data/generator inputs currently remain in ignored audit storage. Supplement v13 now includes the current Unicode notice; the old fixed v5 catalog remains unchanged. Connect the external inputs and provenance to the final material delivery without overwriting old fixed kits. [Fontconfig, HarfBuzz and libunibreak references](NATIVE_FONT_DATA_AUDIT.md) are now identified separately. The named embedded-code/NOTICE cases, final source access and all installer/quality gates remain open.
