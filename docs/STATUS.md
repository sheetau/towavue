...
APNG display/export compositing is aligned at 98e947f. Texture caching now stops retaining otherwise-unused original pixels; the bounded 100-JPEG comparison below demonstrates a browsing-memory improvement. Next: remaining animation-preserving export/conversion policy, cold/other-format image performance, and actual OS-input validation when available. Do not recreate completed APNG or cache-lifetime work.
...
- Verified code: **2026-09-12 weak original references in texture caching**, based on 98e947f. Full suite: 699 passed (app 391, core 67, runtime 237, integration 4), 22 ignored; fmt, Clippy and Release passed. After adding test-only ownership comparison, focused cache regression and Clippy passed again; optimized cache and GPU navigation tests passed. These are not physical-input checks.
...
| E01 | Audio-only output, normalization/channel options, metadata UI, PNG/JPEG/static WebP fields, supported APNG export including posters and shared display/export compositing | GIF/WebP/AVIF animation-preserving export and conversion policy; full 16-bit/ICC; AlbumArtist/EXIF/IPTC/COM/unknown or Extended XMP and cross-format metadata |
...
- **2026-09-12 — texture-cache ownership:** removed redundant original-pixel retention without changing cache budgets or active-tab state. Verification and controlled memory/latency tradeoffs are above; continue broader feature/performance work, not distribution.
