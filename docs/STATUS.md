...
APNG, GIF and WebP same-format saves now preserve animation; WebP also shares display edits and supports XMP changes. Next: AVIF animation-preserving export and cross-format conversion policy, long-animation cost, GIF over-budget palette quality/metadata, cold/other-format image performance, and actual OS-input validation when available. Reuse the completed APNG/GIF/WebP preservation and texture-cache ownership work below.
...
- Verified code: **2026-09-12 animated WebP saving**, based on db2fd07. Full workspace: 713 passing tests (app 392, core 67, runtime 250, integration 4), 22 ignored. Subsequent encoded-frame bound and added raster/failure cases passed all 10 WebP regressions. fmt, Clippy and Release passed. These are not physical-input, all-format quality, or long-animation performance checks.
...
- **2026-09-12 — animated WebP saves:** replaced explicit rejection with streaming lossless snapshots, shared display edits and XMP support; source/target protection and app save lifecycle tested. Continue AVIF/conversion, quality/performance and native UI validation, not distribution.
...
- **98e947f — APNG display/export parity:** shared compositing removes posterless OVER rounding differences; exact edited-frame and both-poster-layout failure regressions pass.
