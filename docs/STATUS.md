...
APNG, GIF and WebP same-format saves now preserve animation; WebP also shares display edits and supports XMP changes. AVIF display now retains the last-frame/first-preview source duration, but AVIF animation export is still unimplemented. Next: AVIF frame/timing/loop/alpha-preserving export and cross-format policy, long-animation cost, GIF over-budget palette quality/metadata, image performance and actual OS-input validation. Reuse the evidence below.
...
- Verified code: **2026-09-12 AVIF display timing**, based on 9d8a564. Full workspace: 715 passing tests (app 392, core 67, runtime 252, integration 4), 22 ignored. fmt, Clippy and Release passed. These are not AVIF export, physical-input, all-format quality, or long-animation performance checks.
...
| AVIF display timing | `animated_format_previews_match_the_original_first_frame` reproduced expected [500,500] ms as [500,100]. Software VideoFrame now carries positive decoder duration; both the tail and first-only preview use it. `avif_variable_frame_timing_keeps_the_tail_and_first_preview_duration` checks [125,750,500] ms with loop settings 0/1/3; packet duration conversion validates time bases | Not AVIF export or playback loop-count support. Export still forces one frame and `-still-picture 1`. The pinned FFmpeg AVIF muxer exposes `-loop`; movenc uses elst repetition flags and tkhd duration for loops, not ordinary metadata tags. Future export must preserve controls and auxiliary alpha rather than merely removing the one-frame limit |
...
- **2026-09-12 — AVIF display timing:** replaced the fabricated 100 ms terminal/preview duration with decoded source timing; CFR/VFR regressions pass. AVIF export remains next work, not completed by this fix.
...
- **5f720b2 — texture-cache ownership:** removed redundant original-pixel retention without changing cache budgets or active-tab state. Controlled memory/latency tradeoffs are above.
