...
APNG display/export compositing is aligned at 98e947f; texture-cache ownership improved at 5f720b2. GIF→GIF saving now preserves animation instead of exporting only one frame. Next: WebP/AVIF animation-preserving export/conversion policy, GIF palette/metadata fidelity, cold/other-format image performance, and actual OS-input validation when available. Do not recreate completed APNG, GIF frame/control preservation, or cache-lifetime work.
...
- Verified code: **2026-09-12 GIF animation-preserving export**, based on 5f720b2. Workspace suites plus focused reruns cover 704 passing tests (app 391, core 67, runtime 242, integration 4), 22 ignored. One stale APNG UI-text assertion was corrected and rerun; subsequent GIF changes passed their focused suite. fmt, Clippy and Release passed. These are not physical-input or all-format quality checks.
...
| E01 | Audio-only output, normalization/channel options, metadata UI, PNG/JPEG/static WebP fields, APNG export with posters/shared compositing, GIF frame/timing/repeat-preserving saves | WebP/AVIF animation-preserving export and conversion policy; GIF palette/extension/metadata fidelity and long-animation cost; full 16-bit/ICC; AlbumArtist/EXIF/IPTC/COM/unknown or Extended XMP and cross-format metadata |
...
| GIF animation export | `export_gif.rs` first reproduced three frames saved as one. Five new tests cover exact source delays/repeat, limited-palette edited pixels, transparent partial frames/disposal/duplicates/resave, static conversion, count/canvas boundaries, cancellation/source-change/corruption/occupied-stage target protection | Invalid LZW with intact frame controls was accepted by FFmpeg even with `-xerror`; indexed source decoding with end-code validation now rejects it before publishing. Only one indexed frame is retained at a time; palette conversion and additional validation/rewrite cost are not lossless or performance guarantees. Unknown extensions/metadata and animation cross-format conversion remain separate |
...
- **2026-09-12 — GIF animation saving:** implemented frame/control preservation and strict damaged-source protection; corrected the stale APNG compositing explanation. Verification is above. Continue remaining format/UI/performance work, not distribution.
