...
| V03 | timelineの時間選択・範囲再生・内外削除・連結、部分音量／速度、rubber-band | 基盤実装中。coreに単一sourceの区間列／編集時刻↔source時刻／Keep・Delete・部分音量・StretchとUndo/Redo replayを追加。runtime exportは同じplanのframe/sample選択、局所／master音量・速度、連結を使用。元画素・PCM参照、offset PTS・音声なし・取消／既存target保護を回帰で確認。まだUIへ公開しておらず、現在のtrim gripは要求と異なる。次は再生clock／queue・waveformの編集時間軸と選択UI／rubber-band／shortcutを接続し、実操作と保存再openの一致まで検証する |
