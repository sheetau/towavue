...
| E01 | metadata書換、音声抽出、normalize、stereo／mono export | 部分実装。動画の音声のみ別名出力、normalize／Mono・Stereo変換の設定UIとSave連携・source別session内保持は接続済み。実PCM・取消／guard・実GPU復旧を確認。個別metadata書換、通常window／全codec品質は未完 |
