...
| U08 | tab dragの連続性、window分離／結合、filmstripから分離、drop indicator | 一部実装。tab本体の追従、隣接tab即時投影、端の横scroll、release時だけの並べ替えと取消に対応。3幅×3密度、batch／source・構成・画面変化、実GPU往復・media上取消を確認。外への分離は既存の別processへのguard付きpath detachのみ。連続したwindow分離／結合・状態移送、filmstrip分離と通常window／物理入力／mixed-DPIを未完として扱う |
