...
| U08 | tab dragの連続性、window分離／結合、filmstripから分離、drop indicator | 一部実装。tab追従・隣接投影・端scroll・release確定／取消、filmstripのpath-only新window要求を確認。音声／動画の外分離を同一hostのlive session／未保存編集移送へ接続。非表示HWNDでactive／非active動画・無音音声・再生／停止・反復移送・focus・repeat／shuffle・不変origin配送・初期化前後の失敗保持を確認。画像context間移送、filmstripの同host化／子process実起動、通常のwindow間drop／結合indicator、可視window／物理入力／mixed-DPIは未完 |
