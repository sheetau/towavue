...
| U07 | tabごとの全表示／再生状態保持、背景音声・複数動画、非activeの表示負荷抑制 | 一部実装。読み込み済み画像のRGBA／texture・zoom／pan／selection・読書設定／ページ・編集元をtab IDで保持し、復帰時の再読込／同device再uploadを省く。未完了decode／resizeは再開、一覧scroll等の完全保持は未完。開いた画像はcloseまでpinするためprocess全体のmemory上限ではない。動画／音声はまだsession再構築。単一device維持・背景session所有・audio共存・resource／終了順の検証を続ける |
