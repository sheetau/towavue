...
| U03/U04 font／resize | seek／tab preview時刻はProportionalへ統一済み。通常幅のtab名／list／status path・time・volumeは4倍率の1px往復resizeで不動。狭い音声時計だけ可変領域内中央寄せで0.5px移動を再現し、領域を左寄せへ変更して同回帰を通過。その他font漏れ／実window drag全経路は未完 |
