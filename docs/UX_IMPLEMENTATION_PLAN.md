...
| I06 | preset aspect selection、自由回転、readingの回転／反転alias | readingのR／L配置切替・H／V順反転を接続済み。比率preset7種をEdit menu／palette／Ctrl+Kの後に1～7へ追加し、編集後寸法・SAR・context・履歴・PNG保存画素と実動画cropを回帰確認。自由回転は画像のcore値／非同期raster／export基盤まで実装。0.1度、検証済み外接寸法、透明alpha補間、90度倍数の画素維持、合成履歴の表示用画素とPNG一致、animation・Undo/Redo・tab／stale拒否を回帰確認した。操作UI／hold-drag、動画の単一device表示／SARと保存、通常window入力／appearance・性能は未完 |
