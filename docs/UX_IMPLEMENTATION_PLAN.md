...
| I06 | preset aspect selection、自由回転、readingの回転／反転alias | readingのR／L配置切替・H／V順反転を共通bindingへ接続し、非編集・通常画像のL回転／V反転とのcontext分離を回帰確認。presetと自由回転は未完。90度・反転・正方形／比率保持は存在。任意角度の境界／export契約を別途決める |
