...
| U08 | tab dragの連続性、window分離／結合、filmstripから分離、drop indicator | 一部実装。同hostの通常window間drop／gap挿入線・inactive端scroll／Welcome結合を接続。headlessとOS hitだけを注入した非表示HWNDでdirty画像移送／取消を確認。tab追従・隣接投影・端scroll・release確定／取消、filmstripの独立Openと画像／音声／動画tabのlive state／未保存編集移送を維持。画像frame／画素Arc・不足ページ再開／preview／resample・予算維持、動画／無音音声・repeat／shuffle・不変origin配送、反復移送／focus／GPU復旧／失敗保持を確認。別process入口の所有権、可視window／物理入力／重なり／mixed-DPI・移送速度は未完 |
