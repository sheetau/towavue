...
| U08 | tab dragの連続性、window分離／結合、filmstripから分離、drop indicator | 一部実装。tab追従・隣接投影・端scroll・release確定／取消、filmstripのpath-only新window要求を確認。画像／音声／動画の外分離を同一hostのlive state／未保存編集移送へ接続。画像frame／画素Arc・不足ページ再開／preview／resample・予算維持、動画／無音音声・repeat／shuffle・不変origin配送、非表示HWNDでの反復移送／focus／GPU復旧／失敗保持を確認。filmstripの同host化／子process実起動、通常のwindow間drop／結合indicator、可視window／物理入力／mixed-DPI・移送速度は未完 |
