...
| I05 | 画像／選択範囲のclipboard copy、resize/resample、interpolation | 一部実装。Ctrl+C／menu／paletteの共有copy commandを追加。編集後の原寸frame／selectionをworkerでPNG／DIBV5へ送り、透過RGBAとsource・履歴を保持する。Windows 11の通常アプリで600×800原画・800×600回転・550×600selectionのclipboard画素一致と、paletteの文字copy優先を確認。実OS clipboardの透過RGBA往復は別途明示実行したtestで成功。readingはactive画像のみ。resize/resampleの有界履歴・共通FFmpeg materializer／export filterと透明PNGの4補間一致は検証したが、Ctrl+R・非同期表示・処理済み画素copyへの接続は未完。interpolation／nearest表示も未完 |
