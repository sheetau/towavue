...
| V03 | timelineの時間選択・範囲再生・内外削除・連結、部分音量／速度、rubber-band | 基盤実装中。coreの区間列／編集↔source時刻／Keep・Delete・部分音量・Stretchと履歴replayをexportと再生engineで使用。編集PTS／Seek、同一WASAPIでの区間送り、局所／master速度・音量、非表示／復旧／空planを実装。元RGBA、保存PCMとの1LSB以内の一致、実WASAPIのclock／worker保持、H.264 D3D11VAの編集終端／復旧とCPU transfers0を確認。通常appの履歴・duration／waveform・選択UIは未接続で、現在のtrim gripは要求と異なる。次はその接続とrubber-band／shortcut、実操作と保存再openを検証。長い削除区間のdecode負荷・初期Seek位相・継ぎ目の音質も残る |
