...
| I06 | preset aspect selection、自由回転、readingの回転／反転alias | readingのR／L配置切替・H／V順反転を接続済み。比率preset7種をEdit menu／palette／Ctrl+Kの後に1～7へ追加し、編集後寸法・SAR・context・履歴・PNG保存画素と実動画cropを回帰確認。画像自由回転のcore値／非同期raster／exportに加え、menu／Ctrl+Shift+R／角度dialog／slider／近似配置previewを接続。0.1度、外接寸法、alpha補間、PNG一致、animation・Undo/Redo・tab／stale拒否、取消／0度非編集、focus復帰、custom binding衝突、320×300 scrollを回帰確認した。画像上のhold-drag、動画の単一device表示／SARと保存、通常window入力／appearance・性能は未完 |
