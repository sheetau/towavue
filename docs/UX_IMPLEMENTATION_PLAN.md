...
| I05 | 画像／選択範囲のclipboard copy、resize/resample、interpolation | 一部実装。Ctrl+Cの編集後frame／selection copy、透過RGBA、readingのactive画像限定は検証済み。Ctrl+R／menu／paletteへ幅・高さ／比率固定／4補間のmodalを追加し、非同期materializationとUndo／Redo、処理済み画素copyへ接続。透明PNGの4補間はexportと共通filterで一致。Windows 11ではUIA値設定／keyboard／Cancel、300×400の実保存PNGとcopyの全画素一致、Undoの600×800原画一致とRedo・通常終了を確認。処理失敗／古い結果／animation／復旧textureは自動回帰。resize後の実GPU復旧・混在DPIの追加監査、interpolation／nearest表示は未完 |
