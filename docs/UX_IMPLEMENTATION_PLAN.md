...
| I09 | 選択角の二辺resize／比率保持、方向cursor、範囲内右drag移動、外部click解除、crop preview撤去／範囲zoom | 一部実装。四隅・Shift比率／対角固定・resize／Crosshair cursorと画像内のpixel寸法固定移動／AllScrollを回帰確認。取消・pan／dirty保持も確認。外部click解除とcrop previewから通常zoomへの置換は未対応。可視操作・全gesture／DPI／動画corner品質も未検証 |
