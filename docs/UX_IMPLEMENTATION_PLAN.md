...
| I06 | preset aspect selection、自由回転、readingの回転／反転alias | readingのR／L配置切替・H／V順反転を接続済み。1:1／4:3／3:4／3:2／2:3／16:9／9:16の中央選択をEdit menu／palette／Ctrl+Kの後に1～7へ追加。編集後寸法・動画SARと画素格子・context・custom prefix・非編集・時間選択解除を回帰確認。回転後PNGのcrop／export／再読込は全画素一致、実SAR動画のcrop／Undo/Redoもhidden-windowで通過。通常window入力／appearanceの最終確認と自由回転は未完。任意角度の境界／export契約を別途決める |
