...
$inventoryPath = Join-Path $repositoryRoot 'docs/native-chromaprint-inputs.json'
$inventory = Get-Content -LiteralPath $inventoryPath -Raw -Encoding UTF8 | ConvertFrom-Json
if ($inventory.schema_version -ne 1) { throw 'Unsupported native Chromaprint inventory.' }
...
if ($package.Count -ne 1 -or $package[0].version -ne $inventory.package.version -or
    ...
    throw 'Native Chromaprint package inventory is stale.'
}

function Assert-Material([string]$Path, $Record) {
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) { throw "Missing Chromaprint material: $Path" }
    if ((Get-Item -LiteralPath $Path).Length -ne $Record.bytes -or
        ...
        throw "Chromaprint material checksum mismatch: $Path"
    }
}
...
Assert-Material $LgplLicense $inventory.lgpl
...
if ($LASTEXITCODE -ne 0) { throw 'Chromaprint package metadata extraction failed.' }
& $tar -xf $SourceArchive -C $OutputDirectory @($inventory.source_notices | ForEach-Object { $inventory.source.prefix + $_.name })
if ($LASTEXITCODE -ne 0) { throw 'Chromaprint source notice extraction failed.' }
...
if (-not $recipeText.Contains($inventory.source.sha256) -or -not $recipeText.Contains('-DFFT_LIB=kissfft')) {
    throw 'The recipe does not identify the pinned source and KissFFT backend.'
}
...
Copy-Item -LiteralPath (Join-Path $repositoryRoot 'third-party/CHROMAPRINT-MATERIALS-README.txt') -Destination (Join-Path $OutputDirectory 'README.txt')
...
Write-Output "Native Chromaprint materials: $OutputDirectory"
