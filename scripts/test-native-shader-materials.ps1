...
foreach ($kind in @('consumer', 'dependency', 'nested', 'historical_header', 'opencl_header', 'package', 'recipe', 'duplicate', 'path')) {
    ...
    switch ($kind) {
        ...
        'opencl_header' { ($changed.components | Where-Object name -eq 'opencl-headers').version = '2~2026.05.29-0'; $message = 'Stale native shader dependency mapping.' }
...
Write-Output "Native shader checks passed: $($expected.Count) exact files, arbitrary cwd/repeat, $($inputs.Count) missing/corrupt pairs, nine mapping failures, two incomplete-extraction marker cases and input/output preservation. Evidence: $testRoot"
