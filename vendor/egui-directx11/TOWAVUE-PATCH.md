...
- Compile the original HLSL through D3DCompile at build time, preserving shader
  targets, entry points and optimization flags. Embed Cargo-generated bytecode;
  runtime shader/input-layout creation still uses the caller's device. An exact
  bytecode regression retains the former compiler path only in tests. This
  source-only copy contains no checked-in shader binaries or example media.
