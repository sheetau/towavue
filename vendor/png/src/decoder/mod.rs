...
impl<R: BufRead + Seek> Reader<R> {
    /// Set a safe row override without changing parsing, limits, transformations,
    /// stream history, interlace/frame resets or error validation.
    #[cfg(feature = "towavue-row-filter")]
    pub fn set_row_filter(&mut self, filter: Option<crate::RowFilterFn>) {
        self.unfiltering_buffer.row_filter = filter;
    }
