...
pub(crate) struct UnfilteringBuffer {
    #[cfg(feature = "towavue-row-filter")]
    pub(super) row_filter: Option<crate::RowFilterFn>,
...
impl UnfilteringBuffer {
    ...
    pub fn unfilter_curr_row_in_place(
        ...
        let handled = self
            .row_filter
            .is_some_and(|f| f(filter.into(), bpp as u8, prev, row));
        #[cfg(not(feature = "towavue-row-filter"))]
        let handled = false;
        if !handled {
    ...
    pub fn unfilter_curr_row_using_scratch_buffer(
        ...
        let handled = self
            .row_filter
            .is_some_and(|f| f(filter.into(), bpp as u8, prev, &mut row));
        #[cfg(not(feature = "towavue-row-filter"))]
        let handled = false;
        if !handled {
...
#[cfg(all(test, feature = "towavue-row-filter"))]
mod row_filter_tests {
    ...
    fn override_paeth(filter: crate::Filter, bpp: u8, previous: &[u8], current: &mut [u8]) -> bool {
        CALLS.with(|calls| calls.set(calls.get() + 1));
        assert_eq!(bpp, 4);
        if !matches!(filter, crate::Filter::Paeth) || previous.is_empty() {
            return false;
        }
        assert_eq!(previous, [0; 20]);
        current.fill(1);
        true
    }

    #[test]
    fn override_preserves_in_place_and_scratch_row_transitions() {
        for scratch in [false, true] {
            CALLS.with(|calls| calls.set(0));
            let mut buffer = UnfilteringBuffer::new(&Info {
                width: 5,
                height: 2,
                color_type: crate::ColorType::Rgba,
                bit_depth: crate::BitDepth::Eight,
                ..Info::default()
            });
            buffer.row_filter = Some(override_paeth);
            buffer.data_stream = vec![0; 42];
            buffer.data_stream[21] = 4;
            buffer.filled = 42;
            buffer.available = if scratch { 0 } else { 42 };
            for expected in [0, 1] {
                if scratch {
                    buffer
                        .unfilter_curr_row_using_scratch_buffer(21, BytesPerPixel::Four)
                        .unwrap();
                } else {
                    buffer
                        .unfilter_curr_row_in_place(21, BytesPerPixel::Four)
                        .unwrap();
                }
                assert_eq!(buffer.prev_row(), [expected; 20]);
            }
            assert_eq!(CALLS.with(Cell::get), 2);
            buffer.reset_prev_row();
            assert!(buffer.prev_row().is_empty());
            buffer.start_frame(21);
            buffer.data_stream = vec![0; 21];
            buffer.data_stream[0] = 5;
            buffer.filled = 21;
            buffer.available = 21;
            assert!(buffer
                .unfilter_curr_row_in_place(21, BytesPerPixel::Four)
                .is_err());
            assert_eq!(
                CALLS.with(Cell::get), 2,
                "invalid filter never reaches override"
            );
        }
    }
}
