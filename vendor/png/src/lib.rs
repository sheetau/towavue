...
/// Optional safe row reconstruction override. Return false without modifying
/// `current` to use the built-in filter. The previous row is empty at frame/pass
/// start; otherwise it has the same length. Byte depth is the PNG filter stride.
#[cfg(feature = "towavue-row-filter")]
pub type RowFilterFn = fn(Filter, u8, &[u8], &mut [u8]) -> bool;
